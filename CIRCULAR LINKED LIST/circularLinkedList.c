...
struct Node * insertAtFirst(struct Node * head, int val){
    //Time complexity of Insertion at first is O(n).
    ...
    //After the end of the while loop, p would be pointing to the last node of the Circular linked list.
    p -> next = ptr;
    ptr -> next = head;
    head = ptr;
...
struct Node * deleteFirst(struct Node * head){
    //Time complexity of Deleting the first node is O(n).
    struct Node * ptr = head;
    struct Node * p = head -> next;
    while (p -> next != head){
        p = p -> next;
...
struct Node * deleteLast(struct Node * head){
    ...
    //After the end of the while loop, ptr would be pointing to the last node of the Circular linked list and p would be the element before ptr.
    p -> next = head;
    free(ptr);
    printf("\nDeletion Successful!\n\n");
    return head;
...
int main(){
    //This is a simple demonstration of Circular linked list with 4 elements.
